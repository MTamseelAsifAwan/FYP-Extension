...
const Tasks = () => {
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-3 gap-10 p-6" style={{ width: '700px', height: '100%' }}>
        {/* Skeleton loaders */}
        {[...Array(9)].map((_, index) => (
  ...
  return (
    <div className="p-6" style={{ width: '700px', height: '100%' }}>
      {viewDetails ? (
        loadingDetails ? (
          <div>
            <button
        ...
        <div className="grid grid-cols-3 gap-6 p-6 overflow-y-scroll" style={{ height: '100%' }}>
          {Object.entries(tasks).map(([key, item]) => (
            <div
              key={key}
              className="relative flex flex-col bg-black bg-opacity-40 backdrop-filter backdrop-blur-sm rounded-3xl p-4 shadow-lg h-40 shadow-white"
